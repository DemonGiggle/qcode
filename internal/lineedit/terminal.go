...
func (t *Terminal) writeLine(line []rune) {
	inEscape := false
	for _, r := range line {
		width := runeColumns(r, &inEscape)
		if width > 0 && t.cursorX+width > t.termWidth {
			t.clearLineToRight()
			t.queue([]rune("\r\n"))
			t.cursorX = 0
			t.cursorY++
...
// position mirrors writeLine, including the unused cell when a wide rune
// cannot fit at the right margin. Positions in line remain rune indexes.
func (t *Terminal) position(line []rune) (x, y int) {
	inEscape := false
	for _, part := range [][]rune{t.prompt, line} {
		for _, r := range part {
			width := runeColumns(r, &inEscape)
			if width > 0 && x+width > t.termWidth {
				x = 0
				y++
			}
			x += width
...
func runeColumns(r rune, inEscape *bool) int {
	if *inEscape {
		if r >= 'a' && r <= 'z' || r >= 'A' && r <= 'Z' {
			*inEscape = false
		}
...
func (t *Terminal) Write(buf []byte) (n int, err error) {
	...
	t.cursorX, t.cursorY = 0, 0
	t.clearLineToRight()
	for t.cursorY < t.maxLine {
		t.move(0, 1, 0, 0)
		t.cursorY++
		t.clearLineToRight()
	}
	t.move(t.cursorY, 0, 0, 0)
	t.cursorY, t.maxLine = 0, 0
