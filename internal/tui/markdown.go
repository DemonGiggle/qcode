...
// writeThinkingStatement keeps structured Markdown inside the thinking block
// aligned with the block's statement marker. Plain statements already get a
// marker from writeStatement; list items and other structured lines need an
...
func (w *MarkdownWriter) writeThinkingStatement(line string) {
	...
	continuation := indent
	if marker, _, ok := listItem(content, w.unicode); ok {
