...
async function downloadExport(line){
  const argument=line.replace(/^\/export(?:\s+|$)/,'').trim();
  try{
    const r=await apiFetch('api/v1/export?mode='+encodeURIComponent(argument),{cache:'no-store'});
    ...
    a.download=match?match[1]:'qcode-session.html';
...
form.onsubmit=e=>{e.preventDefault();const line=input.value.trim();if(!line)return;if(line==='/exit'||line==='/quit'){closed=true;events.close();status.textContent='Disconnected';send.disabled=true;input.disabled=true;waitingRunning=false;drawWaiting();return}if(line==='/clear'){const v=(snapshot.presentation.views||[]).find(v=>v.id===active);cleared[active]=v?v.lines.length:0;input.value='';render();return}if(/^\/export(?:\s|$)/.test(line)){downloadExport(line);return}if(handleCommand(line))return;submitLine(line)};
login();
